					...
					const char *buf, size_t count)
{
	...
	if (value) {
		...
		input_report_abs(input_dev, ABS_MT_POSITION_Y, 1756);
		input_sync(input_dev);
	} else {
		input_mt_slot(input_dev, 0);
...
static DEVICE_ATTR(fts_fod_test, S_IRUGO | S_IWUSR, NULL, fts_fod_test_store);
