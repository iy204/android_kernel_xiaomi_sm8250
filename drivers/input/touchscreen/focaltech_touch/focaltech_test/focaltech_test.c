...
static ssize_t fts_test_data0_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	/*Gets the number of tests in the test library and saves it*/
	int cnt;
	int data_len;
	...
	data_len = test_data.len_store_msg_area + test_data.len_store_data_area;
	cnt = snprintf(buf, PAGE_SIZE, test_data.store_all_data);
	FTS_TEST_FUNC_EXIT();

	return cnt;
}

static ssize_t fts_test_data1_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int data_len = 0;
	int cnt;
	...
	if (data_len - PAGE_SIZE > 0)
		cnt = snprintf(buf, (data_len - PAGE_SIZE) / PAGE_SIZE > 0 ? PAGE_SIZE : (data_len - PAGE_SIZE), test_data.store_all_data + PAGE_SIZE);
	else
		cnt = 0;
	FTS_TEST_FUNC_EXIT();
...
static ssize_t fts_test_result0_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
	...
	cnt = snprintf(buf, PAGE_SIZE, test_data.testresult);
	FTS_TEST_FUNC_EXIT();

	return cnt;
}

static ssize_t fts_test_result1_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
	...
	if (test_data.testresult_len - PAGE_SIZE > 0)
		cnt = snprintf(buf, (test_data.testresult_len - PAGE_SIZE) / PAGE_SIZE > 0 ? PAGE_SIZE : (test_data.testresult_len - PAGE_SIZE), test_data.testresult + PAGE_SIZE);
	else
		cnt = 0;
...
static ssize_t fts_test_result2_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
	...
	if (test_data.testresult_len - 2 * PAGE_SIZE > 0)
		cnt = snprintf(buf, (test_data.testresult_len - 2 * PAGE_SIZE) / PAGE_SIZE > 0 ? PAGE_SIZE : (test_data.testresult_len - 2 * PAGE_SIZE), test_data.testresult + 2 * PAGE_SIZE);
	else
		cnt = 0;
	FTS_TEST_FUNC_EXIT();
...
static ssize_t fts_test_result3_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
	...
	if (test_data.testresult_len - 3 * PAGE_SIZE > 0)
		cnt = snprintf(buf, (test_data.testresult_len - 3 * PAGE_SIZE) / PAGE_SIZE > 0 ? PAGE_SIZE : (test_data.testresult_len - 3 * PAGE_SIZE), test_data.testresult + 3 * PAGE_SIZE);
	else
		cnt = 0;
	FTS_TEST_FUNC_EXIT();
...
static ssize_t fts_test_result4_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
	...
	if (test_data.testresult_len - 4 * PAGE_SIZE > 0)
		cnt = snprintf(buf, (test_data.testresult_len - 4 * PAGE_SIZE) / PAGE_SIZE > 0 ? PAGE_SIZE : (test_data.testresult_len - 4 * PAGE_SIZE), test_data.testresult + 4 * PAGE_SIZE);
	else
		cnt = 0;
	FTS_TEST_FUNC_EXIT();
...
static ssize_t fts_test_result5_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
	...
	if (test_data.testresult_len - 5 * PAGE_SIZE > 0)
		cnt = snprintf(buf, (test_data.testresult_len - 5 * PAGE_SIZE) / PAGE_SIZE > 0 ? PAGE_SIZE : (test_data.testresult_len - 5 * PAGE_SIZE), test_data.testresult + 5 * PAGE_SIZE);
	else
		cnt = 0;
	FTS_TEST_FUNC_EXIT();
...
static ssize_t fts_test_result6_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
	...
	if (test_data.testresult_len - 6 * PAGE_SIZE > 0)
		cnt = snprintf(buf, (test_data.testresult_len - 6 * PAGE_SIZE) / PAGE_SIZE > 0 ? PAGE_SIZE : (test_data.testresult_len - 6 * PAGE_SIZE), test_data.testresult + 6 * PAGE_SIZE);
	else
		cnt = 0;
	FTS_TEST_FUNC_EXIT();
...
static ssize_t fts_test_result7_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
	...
	if (test_data.testresult_len - 7 * PAGE_SIZE > 0)
		cnt = snprintf(buf, (test_data.testresult_len - 7 * PAGE_SIZE) / PAGE_SIZE > 0 ? PAGE_SIZE : (test_data.testresult_len - 7 * PAGE_SIZE), test_data.testresult + 7 * PAGE_SIZE);
	else
		cnt = 0;
...
static DEVICE_ATTR(fts_test_data0, S_IRUGO, fts_test_data0_show, NULL);
static DEVICE_ATTR(fts_test_data1, S_IRUGO, fts_test_data1_show, NULL);
static DEVICE_ATTR(fts_test_result0, S_IRUGO, fts_test_result0_show, NULL);
static DEVICE_ATTR(fts_test_result1, S_IRUGO, fts_test_result1_show, NULL);
static DEVICE_ATTR(fts_test_result2, S_IRUGO, fts_test_result2_show, NULL);
static DEVICE_ATTR(fts_test_result3, S_IRUGO, fts_test_result3_show, NULL);
static DEVICE_ATTR(fts_test_result4, S_IRUGO, fts_test_result4_show, NULL);
static DEVICE_ATTR(fts_test_result5, S_IRUGO, fts_test_result5_show, NULL);
static DEVICE_ATTR(fts_test_result6, S_IRUGO, fts_test_result6_show, NULL);
static DEVICE_ATTR(fts_test_result7, S_IRUGO, fts_test_result7_show, NULL);

/* add your attr in here*/
static struct attribute *fts_test_attributes[] = {
	&dev_attr_fts_test.attr,
	&dev_attr_fts_test_data0.attr,
	&dev_attr_fts_test_data1.attr,
	&dev_attr_fts_test_result0.attr,
	&dev_attr_fts_test_result1.attr,
	&dev_attr_fts_test_result2.attr,
	&dev_attr_fts_test_result3.attr,
	&dev_attr_fts_test_result4.attr,
	&dev_attr_fts_test_result5.attr,
	&dev_attr_fts_test_result6.attr,
	&dev_attr_fts_test_result7.attr,
	NULL
};
